...
if ~ isempty(b4_e_dts)
    ...
    before_errs_msg = cellfun(@(p)p.message, before_errs, 'UniformOutput', false);
    ...
    disp('--------------report new----------------');
    exp_no = [models.exp_no];
    b4_err_experiments = exp_no(is_exception);
    if size( before_errs_ids')~=size( idx')
        disp(table(idx', b4_err_experiments, before_errs_ids,before_errs_msg));
        writetable(table(idx', b4_err_experiments, before_errs_ids,before_errs_msg),'reslut.xls');
    else
        disp(table(idx', b4_err_experiments, before_errs_ids,before_errs_msg));
        writetable(table(idx', b4_err_experiments, before_errs_ids,before_errs_msg),'reslut.xls');
    end
...
if any(is_comp_e)
    ...
    l.error('errored %s',strjoin(cellfun(@(p) utility.get_error(p), ex_obs, 'UniformOutput', false), '\n'));
